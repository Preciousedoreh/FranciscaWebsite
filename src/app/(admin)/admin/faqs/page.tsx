...
export default function FAQsListPage() {
  ...
  const fetchItems = async () => {
    try { setItems(await faqsApi.getAll()); } catch { /* empty */ } finally { setLoading(false); }
  };
    ...
    <>
      ...
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <Link href="/admin/faqs/new" className="flex items-center gap-2 px-4 py-2 bg-[#FFD11A] text-[#1B2A4A] rounded-lg font-medium text-sm hover:bg-[#e6bc00] transition">
            <FiPlus size={16} /> New FAQ
          </Link>
        </div>
        <div className="bg-white rounded-xl shadow-sm">
          <DataTable columns={columns} data={items} editPath="/admin/faqs" onDelete={setDeleteId} loading={loading} />
        </div>
      </div>
      <DeleteConfirmModal open={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={handleDelete} title="this FAQ" />
    </>
  );
}
